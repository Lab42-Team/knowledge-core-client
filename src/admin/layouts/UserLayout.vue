<template>
  <n-card :title="$t('PAGE.USERS.NAME')" size="medium" :bordered="true">
  ...
  computed: {
    breadcrumbRoutes() {
      ...
      const routes = this.$route.matched
          .filter(route => route.name !== 'UserList') // Пропуск UserList
          .map((route) => {
            let name = this.$t(route.meta.breadcrumb); // Перевод ключа из meta.breadcrumb
            if (route.name === 'UserShow') {
              name = `${this.$t('PAGE.USERS.VIEW')} №${this.$route.params.id}`;
            } else if (route.name === 'UserEdit') {
              name = `${this.$t('PAGE.USERS.EDIT')} №${this.$route.params.id}`;
            } else if (route.name === 'AddUser') {
              name = this.$t('PAGE.USERS.CREATE');
            }
            return {
              name,
