<template>
  <n-card :title="$t('PAGE.KNOWLEDGE_CORE.NAME')" size="medium" :bordered="true">
  ...
  computed: {
    breadcrumbRoutes() {
      ...
      const routes = this.$route.matched
          .filter(route => route.name !== 'KnowledgeCoreShow') // Пропуск KnowledgeCoreList
          .map((route) => {
            let name = this.$t(route.meta.breadcrumb); // Перевод ключа из meta.breadcrumb
            if (route.name === 'KnowledgeCoreEdit') {
              name = `${this.$t('PAGE.KNOWLEDGE_CORE.EDIT')}`;
            }
            return {
              name,
