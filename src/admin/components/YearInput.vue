...
export default {
  ...
  watch: {
    ...
    modelValue(newValue) {
      if (newValue) {
        this.formattedDate = dayjs(newValue).format('YYYY');
  ...
  methods: {
    ...
    initDatepicker(locale) {
      ...
      if (this.modelValue) {
        this.formattedDate = dayjs(this.modelValue).format('YYYY');
      } else {
        // Для Create.vue по умолчанию текущая дата
        const today = dayjs();
        this.formattedDate = today.format('YYYY');
        this.$emit('update:modelValue', today.format('YYYY-01-01 00:00:00'));
      }
      ...
      this.datepickerInstance = new AirDatepicker(inputElement, {
        ...
        onSelect: ({ date }) => {
          if (date) {
            const apiDate = dayjs(date).format('YYYY-01-01 00:00:00'); // Формат для API
            this.formattedDate = dayjs(date).format('YYYY');
