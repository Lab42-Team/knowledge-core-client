<template>
  <n-card :title="$t('PAGE.PROJECTS.CREATE')" size="medium" :bordered="true" class="panel-card">
    <n-form
        ref="formRef"
        :model="project"
        :rules="rules"
    ...
    <n-alert v-if="errorList.length" type="error" class="mt-3">
      <div>
        {{ $t('MESSAGE.PROJECTS.ERROR.CREATING') }}:
        <ul>
...
import { createProject, getProjectTypes, getProjectStatuses } from '@/admin/api/project';
...
export default {
  ...
  watch: {
    '$i18n.locale': {
      handler() {
        this.loadTypes(); // Перезагрузка статусов при смене локализации
        this.loadStatuses(); // Перезагрузка типов при смене локализации
      },
  ...
  methods: {
    ...
    async loadCurrentUser() {
      try {
        ...
        this.project.users = [user.id];
      } catch (err) {
        console.error('Ошибка загрузки пользователя:', err);
        //this.errorList.push(this.$t('MESSAGE.PROJECTS.ERROR.FAILED_TO_FETCH_USER'));
      }
    },
  ...
  async mounted() {
    await Promise.all([
      ...
      this.loadCurrentUser() // Загружаем ID пользователя при монтировании
    ]);
  },
};
</script>
