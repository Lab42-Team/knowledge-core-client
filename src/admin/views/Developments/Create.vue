<template>
  <n-card :title="$t('PAGE.DEVELOPMENTS.CREATE')" size="medium" :bordered="true" class="panel-card">
    <n-form
        ref="formRef"
        :model="developments"
        :rules="rules"
    ...
    <n-alert v-if="errorList.length" type="error" class="mt-3">
      <div>
        {{ $t('MESSAGE.DEVELOPMENTS.ERROR.CREATING') }}:
        <ul>
...
import { createDevelopments } from '@/admin/api/developments';
import YearInput from '@/admin/components/YearInput.vue';
import dayjs from 'dayjs';

export default {
  name: 'DevelopmentsCreate',
  ...
  methods: {
    ...
    async submitForm() {
      try {
        ...
        //console.log('Отправляемые данные:', this.developments);
        // Выполняем запрос на создание новости
        const response = await createDevelopments(this.developments);
